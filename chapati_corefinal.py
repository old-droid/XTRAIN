...
class ChapatiLMTrainer:
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            # Professional checkpointing every 3 epochs or at the end
    ...
    def get_latest_checkpoint(self) -> Optional[str]:
        """
        Get the path to the most recent checkpoint based on epoch number
        """
        ...
        # Parse checkpoint filenames to extract epoch numbers
        # Format: checkpoint_epoch_{epoch}_loss_{loss}.pkl
        epoch_checkpoints = []
...
def auto_train_chapati_lm():
    ...
    print("\n[CHECKPOINT] Checking for existing checkpoints...")
    ...
    if latest_checkpoint:
        ...
        match = re.search(r"checkpoint_epoch_(\d+)_", filename)
        epoch_num = match.group(1) if match else "unknown"

        print(f"[CHECKPOINT] Found checkpoint at epoch {epoch_num}")
        print(f"[CHECKPOINT] Loading checkpoint from: {latest_checkpoint}")
        trainer.load_checkpoint(latest_checkpoint)
    else:
        print("[CHECKPOINT] No existing checkpoints found, starting fresh training")
